...
2. Download https://github.com/GraphiteHealth/GPS-global/blob/master/bin/setup_openehr_git.sh[the bash script /bin/setup_openehr_git.sh^] into that directory and run it (needs to be Linux, cygwin, or other unix-like environment).

You are now ready to go. From the parent directory, in a bash shell, type:

----
$ ./do_spec_publish -r GCM
----

This will publish the HTML documents for the RM component in `GPS-GCM/docs`. You can do the same for any other component, e.g. BASE, QUERY etc.
...
Components, i.e. GPS, AM, LANG, PROC etc are the units of release. Each component has its own Github repo. A release is _always named_ either:
